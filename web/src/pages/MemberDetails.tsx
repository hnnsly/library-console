...
import { 
  ...
  Calendar, AlertTriangle, BookOpen, Clock, Check 
} from 'lucide-react';
import { members, books, checkoutRecords } from '../data/mockData';
...
const MemberDetails: React.FC = () => {
  ...
  const borrowingHistoryWithDetails = member.borrowingHistory.map(historyItem => {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <motion.div
        ...
        animate="visible"
      >
        <motion.div variants={itemVariants} className="bg-white rounded-lg shadow-sm overflow-hidden mb-6">
          <div className={`h-2 ${
            member.membershipStatus === 'active' ? 'bg-green-500' :
            member.membershipStatus === 'expired' ? 'bg-orange-500' : 'bg-red-500'
          }`}></div>
          ...
          <div className="p-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <div className="flex items-center">
                    <h1 className="text-3xl font-serif font-bold">{member.firstName} {member.lastName}</h1>
              ...
              <div className="mt-4 md:mt-0 flex space-x-3">
                <Link to={`/members/${member.id}/edit`} className="btn btn-secondary flex items-center">
            ...
            {member.fines > 0 && (
              <div className="mt-6 bg-red-50 border border-red-100 rounded-lg p-4 flex items-center">
                ...
                <div>
                  <h3 className="font-medium text-red-800">Outstanding Fines</h3>
        ...
        <motion.div variants={itemVariants} className="bg-white rounded-lg shadow-sm overflow-hidden mb-6">
          ...
          <div className="p-6">
            {borrowedBooksDetails.length > 0 ? (
                      ...
                      return (
                        <tr key={item.bookId}>
                          <td className="px-4 py-4 whitespace-nowrap">
                            <Link to={`/books/${item.bookId}`} className="text-primary-600 hover:underline">
              ...
              <p className="text-gray-500 text-center py-6">This member has no books currently checked out.</p>
            )}
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="bg-white rounded-lg shadow-sm overflow-hidden">
          ...
          <div className="p-6">
            {sortedBorrowingHistory.length > 0 ? (
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead>
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Returned</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
              ...
              <p className="text-gray-500 text-center py-6">This member has no borrowing history.</p>
            )}
          </div>
        </motion.div>
      </motion.div>
